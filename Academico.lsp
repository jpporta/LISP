...
(defun cancelar-matricula (alunos disciplinas bd)
	(if (null bd)
		...
		(if (equal (car disciplinas) (caar bd))
			(if (null (set-difference (cadar bd) alunos :test 'equal))
				(if (null (cddar bd))
					(cancelar-matricula alunos disciplinas (cdr bd))
					(cons (cons (caar bd) (cons nil (cddar bd))) (cancelar-matricula alunos disciplinas (cdr bd)))
				)
				(if (equal (set-difference (cadar bd) alunos :test 'equal) (cadar bd))
					(cons (car bd) (cancelar-matricula alunos disciplinas (cdr bd)))

					(cons (cons (car disciplinas) (cons (set-difference (cadar bd) alunos :test 'equal)
					(cddar bd)))(cancelar-matricula alunos disciplinas (cdr bd)))
				)
			)
			(cons (car bd) (cancelar-matricula alunos disciplinas (cdr bd)))
		)
...
(defun remover-vinculo (professores disciplinas bd)
	(if (null bd)
		nil
		(if (equal (car disciplinas) (caar bd))
			(if (null (set-difference (cddar bd) professores :test 'equal))
				(if (null (cadar bd))
					(remover-vinculo professores disciplinas (cdr bd))
					(cons (cons (caar bd) (cons (cadar bd) nil)) (remover-vinculo professores disciplinas (cdr bd)))
				)
				(if (equal (set-difference (cddar bd) professores :test 'equal) (cddar bd))
					(cons (car bd) (remover-vinculo professores disciplinas (cdr bd)))

					(cons (cons (car disciplinas) (cons (set-difference (cddar bd) professores :test 'equal)
					(cadar bd)))(remover-vinculo professores disciplinas (cdr bd)))
				)
			)
				(cons (car bd) (remover-vinculo professores disciplinas (cdr bd)))
		)
